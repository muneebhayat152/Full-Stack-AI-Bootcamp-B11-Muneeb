...
df = pd.read_csv(
    "FinalAssessment/SalesforceStock/Salesforce (CRM) From 2004 To Dec-2024.csv",
    index_col="Date"
)
...
plt.savefig("FinalAssessment/output_plots/Salesforce_returns.png")
plt.close()

cols = ["Return", "Volume_Ratio", "Vol20", "Mom10"]
sns.heatmap(df[cols].corr(), annot=True, cmap="coolwarm")
plt.title("Salesforce - Correlation Heatmap")
plt.savefig("FinalAssessment/output_plots/Salesforce_corr.png")
plt.close()
...
plt.legend()
plt.title("Salesforce - Price Trend")
plt.savefig("FinalAssessment/output_plots/Salesforce_trend.png")
plt.close()
...
plt.title("Salesforce - Market Regimes (KMeans + PCA)")
plt.savefig("FinalAssessment/output_plots/Salesforce_regimes.png")
plt.close()
...
# ---------- 8. Bonus insight: crisis-period volatility ----------

vol_2008 = df.loc["2008-01-01":"2008-12-31", "Return"].std()
vol_2020 = df.loc["2020-01-01":"2020-12-31", "Return"].std()
vol_overall = df["Return"].std()

print("\n2008 Financial Crisis volatility:", vol_2008)
print("2020 COVID Crash volatility:", vol_2020)
print("Overall historical volatility:", vol_overall)
...
print("\n=== Regression: predicting next-day return (Salesforce) ===")
